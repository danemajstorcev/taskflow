...
export default function UsersPage() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    dispatch(
      addToast({ message: `${form.name} added to team`, variant: "success" }),
    );
  ...
  const handleRemove = (id: string, name: string) => {
    if (id === user.id) {
      dispatch(
        addToast({ message: "You can't remove yourself", variant: "error" }),
      );
      return;
    }
  ...
  const handleRoleChange = (id: string, role: Role) => {
    if (id === user.id) {
      dispatch(
        addToast({ message: "Can't change your own role", variant: "warning" }),
      );
      return;
    }
      ...
      {showForm && (
        ...
        >
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button
              type="submit"
              className="px-4 py-2 text-xs font-bold font-mono text-white"
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              ...
              >
                {["Member", "Role", "Team", "Open Tasks", "Actions"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left text-[10px] font-mono uppercase tracking-widest px-4 py-2.5"
                      style={{ color: "var(--text3)" }}
                    >
                      {h}
                    </th>
              ...
              {users.map((u, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <select
                        ...
                        style={{
                          ...
                          borderColor: ROLE_COLOR[u.role] + "40",
                        }}
                      >
                        <option value="user">User</option>
                        <option value="manager">Manager</option>
                    ...
                    <td
                      className="px-4 py-3.5 text-xs font-mono font-bold"
                      style={{
                        color: open > 0 ? "var(--accent)" : "var(--text3)",
                      }}
                    >
                      {open}
                    </td>
