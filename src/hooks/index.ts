...
export const selectFilteredTasks = createSelector(
  ...
  (tasks, filters): Task[] => {
    ...
    switch (filters.sortBy) {
      ...
      case 'priority': result.sort((a, b) => PRIORITY_WEIGHT[b.priority] - PRIORITY_WEIGHT[a.priority]); break;
...
export const selectDashboardStats = createSelector(
  ...
  (tasks) => ({
    ...
    overdue:    tasks.filter((t) => t.dueDate < new Date().toISOString().slice(0, 10) && t.status !== 'done').length,
  })
);
