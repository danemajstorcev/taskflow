...
export default function DocumentDropzone({ taskId, onUploaded }: Props) {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    if (!ACCEPTED.some((a) => a.slice(1) === ext)) {
      dispatch(addToast({ message: `${file.name}: only PDF, Word, or Excel accepted`, variant: 'error' }));
      return;
    }
    setUploading(true);
    try {
      const dataUrl = await readFileAsDataUrl(file);
      const result  = dispatch(uploadDocument({
        name:       file.name,
  ...
  return (
    ...
    >
      <input ref={inputRef} type="file" accept={ACCEPTED.join(',')} multiple
      ...
      {uploading ? (
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm font-mono" style={{ color: 'var(--text2)' }}>Uploading…</p>
        </div>
      ) : (
        <>
          ...
          <div className="flex justify-center gap-2 mt-3">
            {['PDF', 'DOCX', 'XLSX'].map((t) => (
              <span key={t} className="text-[10px] font-mono font-bold px-2 py-0.5 border"
                style={{ borderColor: 'var(--border)', color: 'var(--text3)' }}>{t}</span>
            ))}
          </div>
        </>
      )}
