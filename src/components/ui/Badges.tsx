...
export function StatusBadge({ status }: { status: Status }) {
  const map: Record<Status, { bg: string; color: string; label: string }> = {
    ...
    'in-progress': { bg: 'var(--info-bg)',   color: 'var(--info)',    label: 'In Progress'},
    'done':        { bg: 'var(--success-bg)',color: 'var(--success)', label: 'Done'       },
  };
  const s = map[status];
  return (
    <span className="inline-flex items-center text-[11px] font-semibold px-2 py-0.5 font-mono tracking-wide"
...
export function PriorityBadge({ priority }: { priority: Priority }) {
  ...
  return (
    <span className="inline-flex items-center text-[11px] font-semibold px-2 py-0.5 font-mono uppercase tracking-wider"
      style={{ background: s.bg, color: s.color }}>
      {priority}
    </span>
  );
